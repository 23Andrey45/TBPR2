from __future__ import annotations

from core.strategies.base import Strategy, ParamSpec, StrategyResult, StrategyContext
from core.trading_logic import Trade, CandleData
from core.dividends_calc import calc_paid_dividends_in_range


def _compute_atr_percent(candles: list[CandleData], window: int) -> list[float]:
    """
    ATR% = ATR / close
    ATR берём как простое среднее true range по окну.
    """
    if not candles:
        return []

    window = max(1, int(window))
    closes = [c.close for c in candles]

    true_ranges: list[float] = []
    for i, c in enumerate(candles):
        if i == 0:
            tr = c.high - c.low
        else:
            prev_close = closes[i - 1]
            tr = max(
                c.high - c.low,
                abs(c.high - prev_close),
                abs(c.low - prev_close),
            )
        true_ranges.append(max(0.0, tr))

    atr_pct: list[float] = []
    for i in range(len(candles)):
        start = max(0, i - window + 1)
        atr = sum(true_ranges[start : i + 1]) / (i - start + 1)
        denom = closes[i] if closes[i] != 0 else 1e-12
        atr_pct.append(atr / denom)

    return atr_pct


def _simulate_grid_atr_adaptive(
    candles: list[CandleData],
    *,
    base_step: float,
    atr_window: int,
    atr_mult: float,
    max_step: float,
    trade_lots: int,
    lot_size: int,
) -> dict:
    """
    Адаптивная сетка:
      step_i = clamp(max(base_step, atr_mult * ATR%_i), max_step)

    Позиция: в лотах
    Денежные расчёты: price(за 1 бумагу) * (lots * lot_size)
    """
    if not candles:
        raise ValueError("Нет свечей")

    lot_size = int(lot_size or 1)
    trade_lots = int(trade_lots)
    if trade_lots <= 0:
        raise ValueError("trade_lots должно быть > 0")

    base_step = float(base_step)
    atr_mult = float(atr_mult)
    max_step = float(max_step)

    atr_pct = _compute_atr_percent(candles, atr_window)

    def step_for_i(i: int) -> float:
        s = max(base_step, atr_mult * atr_pct[i])
        if max_step > 0:
            s = min(s, max_step)
        # защита
        return max(1e-8, s)

    trades: list[Trade] = []
    cash = 0.0
    position_lots = 0

    buys = 0
    sells = 0

    max_pos = 0
    min_pos = 0
    min_cash = 0.0
    max_cash = 0.0

    # старт: BUY по open первой свечи
    first = candles[0]
    last_trade_price = first.open

    cash -= last_trade_price * (trade_lots * lot_size)
    position_lots += trade_lots
    buys += trade_lots
    trades.append(Trade(time=first.time, side="BUY", price=last_trade_price, pos_after=position_lots))

    max_pos = max(max_pos, position_lots)
    min_pos = min(min_pos, position_lots)
    min_cash = min(min_cash, cash)
    max_cash = max(max_cash, cash)

    def process_down_segment(t, a: float, b: float, step: float):
        nonlocal cash, position_lots, last_trade_price, buys, max_pos, min_pos, min_cash, max_cash
        seg_lo, seg_hi = b, a
        while True:
            level = last_trade_price * (1.0 - step)
            if not (seg_lo <= level <= seg_hi):
                break

            cash -= level * (trade_lots * lot_size)
            position_lots += trade_lots
            last_trade_price = level
            buys += trade_lots
            trades.append(Trade(time=t, side="BUY", price=level, pos_after=position_lots))

            max_pos = max(max_pos, position_lots)
            min_pos = min(min_pos, position_lots)
            min_cash = min(min_cash, cash)
            max_cash = max(max_cash, cash)

    def process_up_segment(t, a: float, b: float, step: float):
        nonlocal cash, position_lots, last_trade_price, sells, max_pos, min_pos, min_cash, max_cash
        seg_lo, seg_hi = a, b
        while True:
            level = last_trade_price * (1.0 + step)
            if not (seg_lo <= level <= seg_hi):
                break

            cash += level * (trade_lots * lot_size)
            position_lots -= trade_lots  # шорт допустим, как и в базовой сетке
            last_trade_price = level
            sells += trade_lots
            trades.append(Trade(time=t, side="SELL", price=level, pos_after=position_lots))

            max_pos = max(max_pos, position_lots)
            min_pos = min(min_pos, position_lots)
            min_cash = min(min_cash, cash)
            max_cash = max(max_cash, cash)

    # основной цикл
    for i, c in enumerate(candles):
        step_i = step_for_i(i)

        o, h, l, cl = c.open, c.high, c.low, c.close
        path = [o, h, l, cl] if cl >= o else [o, l, h, cl]

        for p0, p1 in zip(path, path[1:]):
            if p1 > p0:
                process_up_segment(c.time, p0, p1, step_i)
            elif p1 < p0:
                process_down_segment(c.time, p0, p1, step_i)

    last_price = candles[-1].close
    equity = cash + position_lots * last_price * lot_size

    return dict(
        cash=cash,
        equity=equity,
        position_lots=position_lots,
        trades=trades,
        buys=buys,
        sells=sells,
        max_pos=max_pos,
        min_pos=min_pos,
        min_cash=min_cash,
        max_cash=max_cash,
        last_price=last_price,
        avg_step=sum(step_for_i(i) for i in range(len(candles))) / len(candles),
        max_step_used=max(step_for_i(i) for i in range(len(candles))),
    )


class GridAtrAdaptiveStrategy(Strategy):
    @property
    def strategy_id(self) -> str:
        return "grid_atr"

    @property
    def strategy_name(self) -> str:
        return "Grid: ATR-adaptive (шире при импульсах)"

    def param_specs(self) -> list[ParamSpec]:
        return [
            ParamSpec("base_step", "Base step (0.01=1%)", "float", 0.01, min=0.0001, max=0.5),
            ParamSpec("atr_window", "ATR window", "int", 14, min=1, max=10_000),
            ParamSpec("atr_mult", "ATR multiplier", "float", 1.0, min=0.0, max=100.0),
            ParamSpec("max_step", "Max step (cap)", "float", 0.05, min=0.0001, max=1.0),
            ParamSpec("lot", "Размер сделки (лоты)", "int", 1, min=1, max=1_000_000),
        ]

    def run(self, candles, params: dict, context: StrategyContext) -> StrategyResult:
        inst = context.instrument
        lot_size = int(getattr(inst, "lot", 1) or 1)

        sim = _simulate_grid_atr_adaptive(
            candles,
            base_step=params["base_step"],
            atr_window=params["atr_window"],
            atr_mult=params["atr_mult"],
            max_step=params["max_step"],
            trade_lots=params["lot"],
            lot_size=lot_size,
        )

        max_invested = max(0.0, -sim["min_cash"])

        metrics = {
            "Свечей": str(len(candles)),
            "avg_step": f"{sim['avg_step']:.6f}",
            "max_step_used": f"{sim['max_step_used']:.6f}",
            "Сделок всего": str(len(sim["trades"])),
            "Покупок (лоты)": str(sim["buys"]),
            "Продаж (лоты)": str(sim["sells"]),
            "Cash (баланс денег)": f"{sim['cash']:.4f}",
            "Equity / прибыль": f"{sim['equity']:.4f}",
            "Финальная позиция (лоты)": str(sim["position_lots"]),
            "Макс. позиция (лоты)": str(sim["max_pos"]),
            "Мин. позиция (лоты)": str(sim["min_pos"]),
            "Макс. вложение (денег)": f"{max_invested:.4f}",
        }

        return StrategyResult(
            strategy_id=self.strategy_id,
            strategy_name=self.strategy_name,
            params_used=params,
            metrics=metrics,
            extra={"trades": sim["trades"]},
        )


class GridAtrAdaptiveDividendsStrategy(Strategy):
    @property
    def strategy_id(self) -> str:
        return "grid_atr_div"

    @property
    def strategy_name(self) -> str:
        return "Grid: ATR-adaptive + дивиденды(paid)"

    def param_specs(self) -> list[ParamSpec]:
        return [
            ParamSpec("base_step", "Base step (0.01=1%)", "float", 0.01, min=0.0001, max=0.5),
            ParamSpec("atr_window", "ATR window", "int", 14, min=1, max=10_000),
            ParamSpec("atr_mult", "ATR multiplier", "float", 1.0, min=0.0, max=100.0),
            ParamSpec("max_step", "Max step (cap)", "float", 0.05, min=0.0001, max=1.0),
            ParamSpec("lot", "Размер сделки (лоты)", "int", 1, min=1, max=1_000_000),
        ]

    def run(self, candles, params: dict, context: StrategyContext) -> StrategyResult:
        inst = context.instrument
        kind = (getattr(inst, "kind", "") or "").lower()
        lot_size = int(getattr(inst, "lot", 1) or 1)

        sim = _simulate_grid_atr_adaptive(
            candles,
            base_step=params["base_step"],
            atr_window=params["atr_window"],
            atr_mult=params["atr_mult"],
            max_step=params["max_step"],
            trade_lots=params["lot"],
            lot_size=lot_size,
        )

        div_paid = 0.0
        if kind in ("share", "etf") and context.dividends and candles and hasattr(candles[0].time, "tzinfo"):
            div_res = calc_paid_dividends_in_range(
                context.dividends,
                sim["trades"],
                start_pos_lots=0,
                lot_size=lot_size,
                range_start=candles[0].time,
                range_end=candles[-1].time,
            )
            div_paid = div_res.paid

        equity_with_div = sim["equity"] + div_paid
        max_invested = max(0.0, -sim["min_cash"])

        metrics = {
            "Свечей": str(len(candles)),
            "avg_step": f"{sim['avg_step']:.6f}",
            "max_step_used": f"{sim['max_step_used']:.6f}",
            "Equity (без див)": f"{sim['equity']:.4f}",
            "Дивиденды (paid)": f"{div_paid:.4f}",
            "Equity (с див)": f"{equity_with_div:.4f}",
            "Сделок": str(len(sim["trades"])),
            "Финальная позиция (лоты)": str(sim["position_lots"]),
            "Макс. позиция (лоты)": str(sim["max_pos"]),
            "Мин. позиция (лоты)": str(sim["min_pos"]),
            "Макс. вложение (денег)": f"{max_invested:.4f}",
        }

        return StrategyResult(
            strategy_id=self.strategy_id,
            strategy_name=self.strategy_name,
            params_used=params,
            metrics=metrics,
            extra={"trades": sim["trades"]},
        )
